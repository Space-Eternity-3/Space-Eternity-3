...
public class SC_object_holder : MonoBehaviour
{
    ...
    public bool generation_done = false;
    ...
    void GenerateStep(int step_size)
    {
        ...
        if(summoned > max_summoned) max_summoned = summoned;
        ...
        if(Objects[j_current]!=null)
        {
            ...
            {
                summoned++;
                SC_seon_remote ser = trn.parent.GetComponent<SC_seon_remote>();
                if(ser!=null)
				{
					Vector3 catch_up = ser.transform.localPosition - ser.localDefault;
					trn.transform.position += catch_up;
				}
            }
        }
